...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .msg.user .bubble {
    background: linear-gradient(135deg, rgba(30, 60, 100, 0.92), rgba(70, 30, 110, 0.92));
  ...
  .sources-label {
    ...
    text-transform: uppercase; color: var(--accent-blue); opacity: 0.8;
  ...
  .source-item:hover { background: rgba(20, 40, 80, 0.95); border-color: rgba(79,163,224,0.5); }
  .source-header {
    display: flex; align-items: center; justify-content: space-between; gap: 8px;
  }
  .source-title {
    font-size: 12.5px; font-weight: 600; color: #7dd3fc;
  ...
  .source-link:hover { background: rgba(168,85,247,0.25); color: #d8b4fe; }
  .source-snippet {
...
function finalizeAssistantMsg(msgEl, answer, sources) {
  ...
  if (sources && sources.length > 0) {
    ...
    sourcesEl.innerHTML = `<div class="sources-label">📄 Retrieved sources (${sources.length})</div>` +
      sources.map(s => {
        ...
        const linkHtml = s.link
          ...
          : '';
        const footerHtml = scoreVal != null ? `
          <div class="source-footer">
            <span class="source-score">Relevance: ${scoreVal}%</span>
